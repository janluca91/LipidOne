...
if ($firstrow =~ /\t/) {
	...
	inserisci_tabella_input(\@table,\@first_column);
...
$radio7=$analisi->insert('Prima::Radio',
          ...
          onClick => sub {$options_ERROR->destroy if $options_ERROR; &insert_options_ERROR; $label_classi->destroy if $label_classi; $options_CLASS->destroy if $options_CLASS; $options_CLASS_CHAINS->destroy if ($options_CLASS_CHAINS); insert_options_CLASS(7,\@names,\@first_column);},
);
                 	     ...
                 	     for ( 0 .. $options_CLASS-> count - 1)
                             ...
                             if ($numcliccati==1)
                   	        ...
                   	        for ( 0 .. $options_CLASS-> count - 1)
                                   {$first_clicked=$_ if $options_CLASS-> button($_); }
                                $clicked_text=$options_CLASS->get_item_text($first_clicked);
                                &populate_menu($clicked_text,\@first_column);
                                }
                             elsif ($numcliccati > 1)
                                {
                                $options_CLASS->button($first_clicked,0);
...
sub collect_options()
{
	...
	if ($radio1->checked){
		...
		$titolo_classe='';
		&crea_file_gnuplot(1,\@elenco_gruppi,$titolo_classe);
	}
	if ($radio2->checked) {
		...
		if ($options_CLASS_CHAINS_1->checked){
			...
			$titolo_classe='';
			&crea_file_gnuplot(2,\@elenco_gruppi,$titolo_classe);
		}
	...
	if ($radio3->checked) {
		 ...
		 if ($options_CLASS_CHAINS_1->checked)
			...
			$options_CLASS=() if ($options_CLASS);
			$titolo_classe='';
			&crea_file_gnuplot(3,\@elenco_gruppi,$titolo_classe);
			}
		if ($options_CLASS_CHAINS_2->checked) {
			 @elenco_classi= &CLASSES();
			 if (scalar(@elenco_classi) == 1)
	 ...
	 if ($radio4->checked) {
		 ...
		 $titolo_classe=$elenco_classi[0];
		 &crea_file_gnuplot(4,\@elenco_gruppi,$titolo_classe) if (scalar(@elenco_classi) == 1);
	}
	 if ($radio5->checked) {
		 $titolo_analisi=$radio5->text;
		 $titolo_classe='';
		 &crea_file_gnuplot(5,\@elenco_gruppi,$titolo_classe);
	   }
	 if ($radio6->checked) {
		 $titolo_analisi=$radio6->text;
		 $titolo_classe='';
		 &crea_file_gnuplot(6,\@elenco_gruppi,$titolo_classe);
	  }
	 ...
	 if ($radio7->checked) {
		 ...
		 if ($selected_chain_type eq 'Select chain type:') {Prima::MsgBox::message_box( 'LipidOne', "Please select a chain type" , mb::Ok);}
			...
			&crea_file_gnuplot(7,\@elenco_gruppi,$titolo_classe);
		 }
	  }
}
...
{
  my $param=shift(@_);
  my $title_analysis=shift(@_);
  my $title_class=shift(@_);
  my @elenco_gruppi=@{shift(@_)};
